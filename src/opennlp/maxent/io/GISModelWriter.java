...
/**
 * Abstract parent class for GISModel writers. It provides the persist method which takes care of
 * the structure of a stored document, and requires an extending class to define precisely how the
...
public abstract class GISModelWriter extends AbstractModelWriter {
    ...
    /**
     * Writes the model to disk, using the <code>writeX()</code> methods provided by extending
     * classes.
     * 
     * <p>
     * If you wish to create a GISModelWriter which uses a different structure, it will be necessary
     * to override the persist method in addition to implementing the <code>writeX()</code> methods.
     */
